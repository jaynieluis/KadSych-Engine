...
class TitleState extends MusicBeatState
{
	...
	override public function create():Void
	{
		...
		if(!closedState) {
			...
			var http = new haxe.Http("https://raw.githubusercontent.com/jaynieluis/KadSych-Engine/master/gitVersion.txt");
			
			http.onData = function (data:String)
			{
				updateVersion = data.split('\n')[0].trim();
				var curVersion:String = MainMenuState.kadSychEngineVersion.trim();
	...
	override function beatHit()
	{
		...
		if(!closedState) {
			...
			switch (sickBeats)
			{
				...
				case 5:
					#if PSYCH_WATERMARKS
					createCoolText(['Epic Face Mod', 'by'], -40);
					#else
					createCoolText(['In association', 'with'], -40);
					#end
				case 7:
					addMoreText('jayTF0', -40);
					ngSpr.visible = true;
				// credTextShit.text += '\nNewgrounds';
				case 8:
					deleteCoolText();
				...
				// credTextShit.text += '\nNight';
				case 15:
					addMoreText('Memes'); // credTextShit.text += '\nMemes';
